          ...
          >
            ...
            {/* {allData.length !== 0 &&
              allData.flatMap((item, index) => {
                // Triple each item
                return [item, item, item].map(
                  (duplicatedItem, duplicatedIndex) => (
                    <div className="item" key={`${index}-${duplicatedIndex}`}>
                      <Link
                        to={`/categories/${duplicatedItem.slugcategory.toLowerCase()}`}
                      >
                        <div className="Catcircle">
                          <img
                            src={duplicatedItem.image}
                            alt="Votre image"
                            className="imgCircle"
                          />
                          <p>{duplicatedItem.title}</p>
                        </div>
                      </Link>
                    </div>
                  )
